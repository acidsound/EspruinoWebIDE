...
(function(){
  ...
  var clearTerminal = function() {
    ...
    var currentLine = NodeMCU.Core.Terminal.getInputLine();
    ...
    NodeMCU.callProcessor("terminalClear");
  };
  ...
  var updateTerminal = function() {  
    ...
    for (var y in termText) {
      var line = termText[y];
      if (y == termCursorY) {
        var ch = NodeMCU.Core.Utils.getSubString(line,termCursorX,1);
        line = NodeMCU.Core.Utils.escapeHTML(
            NodeMCU.Core.Utils.getSubString(line,0,termCursorX)) +
            "<span class='terminal__cursor'>" + NodeMCU.Core.Utils.escapeHTML(ch) + "</span>" +
            NodeMCU.Core.Utils.escapeHTML(NodeMCU.Core.Utils.getSubString(line,termCursorX+1));
      } else
        line = NodeMCU.Core.Utils.escapeHTML(line);
      ...
      switch (ch) {
        ...
        default : {
          ...
          termText[termCursorY] = trimRight(
              NodeMCU.Core.Utils.getSubString(termText[termCursorY],0,termCursorX) +
              String.fromCharCode(ch) + 
              NodeMCU.Core.Utils.getSubString(termText[termCursorY],termCursorX+1));
          termCursorX++;
        }
      }
...
// ----------------------------------------------------------------------------------------------------------
  ...
  /// Called when data comes OUT of NodeMCU INTO the terminal
  ...
  function searchData(bytes){
    ...
    if(si >= 0){ 
      ...
      if(ei > 0){
        ...
        NodeMCU.callProcessor("getWatched",receivedData,function(){});
        receivedData = "";
      }
    }
    else{ receivedData = ""; }
  ...
  function grabSerialPort() {
    // Ensure that keypresses go direct to the NodeMCU device
    NodeMCU.Core.Terminal.setInputDataHandler(function(d) {
      NodeMCU.Core.Serial.write(d);
    });
    // Ensure that data from NodeMCU goes to this terminal
    NodeMCU.Core.Serial.startListening(NodeMCU.Core.Terminal.outputDataHandler);
  };
