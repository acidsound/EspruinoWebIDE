...
(function() {
  ...
  function loadConfiguration(callback) {
    chrome.storage.sync.get( "CONFIGS", function (data) {
      ...
      for (var key in value) { 
        if (key=="set") continue;
        NodeMCU.Config[key] = value[key];
        if (NodeMCU.Core.Config.data[key] !== undefined &&
            NodeMCU.Core.Config.data[key].onChange !== undefined)
          NodeMCU.Core.Config.data[key].onChange(value[key]);
      }  
  ...
  function init() {
    ...
    addSection("Communications", { sortOrder:200, description: "Settings for communicating with the NodeMCU Board" });
    addSection("Board", { sortOrder:300, description: "Settings for the NodeMCU Board itself" });
    
  }
  
  function add(name, options) {
    NodeMCU.Core.Config.data[name] = options;
    if (NodeMCU.Config[name] === undefined)
      NodeMCU.Config[name] = options.defaultValue;
  }
  ...
  function getSections() {
    ...
    // add other sections
    for (var i in NodeMCU.Core.Config.data) {
      var c = NodeMCU.Core.Config.data[i];
  ...
  NodeMCU.Config = {};
  NodeMCU.Config.set = function (key, value) {
    if (NodeMCU.Config[key] != value) {
      NodeMCU.Config[key] = value;
      // Do the callback
      if (NodeMCU.Core.Config.data[key] !== undefined &&
          NodeMCU.Core.Config.data[key].onChange !== undefined)
        NodeMCU.Core.Config.data[key].onChange(value);
      // Save to synchronized storage...
      var data = {};
      for (var key in NodeMCU.Config)
        if (key != "set")
          data[key] = NodeMCU.Config[key];
  ...
  function clearAll() { // clear all settings
    ...
    for (var name in NodeMCU.Core.Config.data) {
      var options = NodeMCU.Core.Config.data[name];
      NodeMCU.Config[name] = options.defaultValue;
    }
  }
