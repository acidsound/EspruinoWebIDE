...
(function(){
  ...
  function getNodeMCUPrompt(callback) {
    ...
    var nextStep = function() {
      ...
      NodeMCU.Core.Serial.startListening(prevReader);
      // call our callback
      callback();
    };
  };
  ...
  function htmlTable(obj) {
    var html = '<table>';
    for (var key in obj) {
      html += '<tr><th>'+NodeMCU.Core.Utils.escapeHTML(key)+'</th><td>'+NodeMCU.Core.Utils.escapeHTML(obj[key])+'</td></tr>';
    }
    return html + '</table>';
  }
  ...
  function getURL(url, callback) {
    NodeMCU.callProcessor("getURL", { url : url, data : undefined }, function(result) {
