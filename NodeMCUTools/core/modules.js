...
(function(){
  ...
  function init() {
    NodeMCU.Core.Config.add("MODULE_URL", {
    ...
    NodeMCU.Core.Config.add("MODULE_EXTENSIONS", {
      section : "Communications",
  ...
  // TODO: use NodeMCU.Core.Env.getData().info.builtin_modules
  ...
  function loadModule(requires, fullModuleName, loadedModuleData) {
    ...
    NodeMCU.callProcessor("getModule",
        ...
        function(data) {
          if (data.moduleCode!==undefined) {
            ...
            if(NodeMCU.Core.Utils.isURL(fullModuleName)) {
              modName = fullModuleName.substr(fullModuleName.lastIndexOf("/") + 1).split(".")[0];
              urls = [ fullModuleName ];
            } else {
              modName = fullModuleName;
              urls = NodeMCU.Config.MODULE_EXTENSIONS.split("|").map(function (extension) {
                return NodeMCU.Config.MODULE_URL + "/" + fullModuleName + extension;
              });
            };
            ...
            (function download(urls) {
              if (urls.length==0) {
                NodeMCU.Core.Notifications.warning("Module "+fullModuleName+" not found");
                return dfd.resolve();
              }
              var dlUrl = urls[0];
              NodeMCU.Core.Utils.getURL(dlUrl, function (data) {
                if (data!==undefined) {
                  // we got it!
