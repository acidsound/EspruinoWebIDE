...
(function(){
  ...
  var getBinary = function(url, callback) {
    ...
    NodeMCU.Core.Status.setStatus("Downloading binary...");
    var xhr = new XMLHttpRequest();          
    xhr.responseType = "arraybuffer";
    xhr.addEventListener("load", function () {
      if (xhr.status === 200) {
        NodeMCU.Core.Status.setStatus("Done.");
  ...
  var initialiseChip = function(callback, timeout) {
    if (!NodeMCU.Core.Status.hasProgress())
      NodeMCU.Core.Status.setStatus("Initialising...");
    ...
    var iPoll = setInterval(function() {
      ...
      NodeMCU.Core.Serial.write("\x7f", false);
    }, 70);
    dataReceived = function (c) {
      ...
      if (c==ACK || c==NACK) {
        ...
        clearInterval(iPoll);
        if (!NodeMCU.Core.Status.hasProgress())
          NodeMCU.Core.Status.setStatus("Initialised.");
  ...
  var sendData = function(data, callback, timeout) {
    ...
    NodeMCU.Core.Serial.write(s + String.fromCharCode(chksum), false);
  ...
  var sendCommand = function(command, callback) {
    NodeMCU.Core.Serial.write(String.fromCharCode(command) + String.fromCharCode(0xFF ^ command), false);
    waitForACK(callback);
  };

  var eraseChip = function(callback) {
    NodeMCU.Core.Status.setStatus("Erasing...");
    // Extended erase
  ...
  var writeAllData = function(binary, callback) {      
    ...
    NodeMCU.Core.Status.setStatus("Writing flash...",  binary.byteLength);
    var writer = function(offset) {
      if (offset>=binary.byteLength) {
        NodeMCU.Core.Status.setStatus("Write complete!");
        callback(undefined); // done
        return;
      }
      ...
      writeData(function(err) {
        if (err) { callback(err); return; }
        NodeMCU.Core.Status.incrementProgress(chunkSize);
  ...
  var readAllData = function(binaryLength, callback) {
    ...
    NodeMCU.Core.Status.setStatus("Reading flash...",  binaryLength);
    var reader = function(offset) {
      if (offset>=binaryLength) {
        NodeMCU.Core.Status.setStatus("Read complete!");
  ...
  function flashDevice(url, callback) {
    getBinary(url, function (err, binary) {
      ...
      // add serial listener
      dataReceived = undefined;
      NodeMCU.Core.Serial.startListening(function (readData) {
      ...
      var hadSlowWrite = NodeMCU.Core.Serial.isSlowWrite();
      NodeMCU.Core.Serial.setSlowWrite(false, true/*force*/);
      var oldHandler = NodeMCU.Core.Terminal.setInputDataHandler(function() {
        // ignore keyPress from terminal during flashing
      });      
      var finish = function(err) {
        NodeMCU.Core.Serial.setSlowWrite(hadSlowWrite);
        NodeMCU.Core.Terminal.setInputDataHandler(oldHandler);
        callback(err);
      };
      // initialise
      initialiseChip(function (err) {
