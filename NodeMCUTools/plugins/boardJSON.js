...
(function(){
  ...
  function init() {
    NodeMCU.Core.Config.add("BOARD_JSON_URL", {
      section : "Board",
      name : "Board JSON URL",
    ...
    NodeMCU.addProcessor("environmentVar", function(env, callback) {
      if (env!==undefined && env.BOARD!==undefined) {
        var jsonPath = NodeMCU.Config.BOARD_JSON_URL+"/"+env.BOARD+".json";
        console.log("Loading "+jsonPath);
        $.getJSON(jsonPath, function(data){
          console.log("Board JSON loaded");
          for (var key in data)
            env[key] = data[key];
          NodeMCU.callProcessor("boardJSONLoaded", env);
