  ...
  function findASMBlocks(code, callback){
    ...
    function match(str, type) {
      if (str!==undefined && tok.str!=str) {
        NodeMCU.Core.Notifications.error("Expecting '"+str+"' but got '"+tok.str+"'. Should have E.asm('arg spec', 'asmline1', ..., 'asmline2'");
        return false;
      }
      if (type!==undefined && tok.type!=type) {
        NodeMCU.Core.Notifications.error("Expecting a "+type+" but got "+tok.type+". Should have E.asm('arg spec', 'asmline1', ..., 'asmline2'");
        return false;
      }      
      tok = lex.next();
      return true;
      ...
      while (tok!==undefined) {
        if (state==0 && tok.str=="E") { state=1; startIndex = tok.startIdx; tok = lex.next();
          ...
          try {
            ...
            NodeMCU.Core.Notifications.error("Assembler failed: "+err);
            return;
          }
