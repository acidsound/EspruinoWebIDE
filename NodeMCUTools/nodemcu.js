...
(function() {
   ...
   *   getURL               - Called with data={url:"http://....", data:undefined) - data should be filled in if the URL is handled (See NodeMCU.Core.Utils.getURL to use this)
  ...
  function init() {    
    ...
    NodeMCU.Core.Config.loadConfiguration(function() {
      ...
      var module;
      for (module in NodeMCU.Core) initModule(module, NodeMCU.Core[module]);
      for (module in NodeMCU.Plugins) initModule(module, NodeMCU.Plugins[module]);
      
      callProcessor("initialised", undefined, function() {
        // We need the delay because of background.js's url_handler...
        setTimeout(function() {
          NodeMCU.initialised = true;
        }, 1000);
      });
    });
  }
